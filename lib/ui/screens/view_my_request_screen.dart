...
class ViewMyRequestScreen extends GetView<ViewMyRequestController> {
  ViewMyRequestScreen({Key? key}) : super(key: key){
    //print(Get.arguments);
  }
                        ...
                        IconButtonWidget(icon: MaterialCommunityIcons
                          ...
                          onPressed: () async {
                            String url = await DynamicLink
                                ...
                                uri: "/request?requestId=${request
                                !.requestId}",metaTag: false);
                            Share.share(url);
                          },),
                      ],
                    ),
...
kHorizontalSpace12,
                                    ...
                                    kVerticalSpace8,
                                    Row(
                                      children: [ Icon(  MaterialCommunityIcons.phone,    size: 16.sp,
                                        color: kAccentColor,),
                                        Expanded(
                                          child: Text(
                                              "${request.phoneNumber}", style: kTextStyle14.copyWith(color: kAccentColor)),
                                        ),
                                      ],
                                    ),
                          ...
                          Row(
                            children: [
                              Expanded(
                                child: ButtonWidget(onPressed: () async {
                                  showConfirmationDialog(onConfirm: () {
                                    var addRequestController = Get.put(
                                        AddRequestController());
                                    addRequestController.deleteRequest(
                                        request: request);
                                    Get.back();
                                  },
                                      title: "Delete",
                              ...
                              Expanded(child: ButtonWidget(onPressed: () async {
                                ...
                                addRequestController.selectedType=addRequestController.typeController.typeModel.value.types.firstWhere((element) => element.typeId==request.typeId);

                                addRequestController.selectedSubtype=addRequestController.selectedType!.subTypes.firstWhere((element) => element.subTypeId==request.subTypeId);
